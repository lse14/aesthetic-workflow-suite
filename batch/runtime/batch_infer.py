...
def _control_wait_if_paused(control) -> None:
    if control is None:
        return
...
def _control_should_stop(control) -> bool:
    if control is None:
        return False
    stop_event = control.get("stop_event")
...
def _score_bucket(value: float, strategy: str) -> int:
    ...
    if s in {"x2_floor_10", "floor_x2_10", "score10"}:
        bucket = int(float(value) * 2.0)
...
def _resolve_model_ref(raw_value: object, *, default_value: str, checkpoint: Path, allow_none: bool = False) -> str | None:
    ...
    if not is_path_like:
        # Keep HF repo ids like "org/model" untouched.
        return raw
...
def _load_runtime(checkpoint: Path, device_override: str | None):
    ...
    runtime_device = (device_override or "cuda").strip().lower()
    if runtime_device == "auto":
        runtime_device = "cuda" if torch.cuda.is_available() else "cpu"
    if runtime_device == "cuda" and not torch.cuda.is_available():
        logging.warning("device='cuda' requested but CUDA is unavailable; fallback to cpu.")
        runtime_device = "cpu"
...
def _infer_records(
    ...
    *,
    ...
    for batch in tqdm(_batched(image_paths, batch_size), total=total_batches, desc="infer", unit="batch"):
        _control_wait_if_paused(control)
        ...
        done_images += len(batch)
        if progress_cb is not None:
            elapsed = max(0.001, time.time() - t0)
            rate = done_images / elapsed
            remain = max(0, total_images - done_images)
            ...
            progress_cb(
                {
                    ...
                    "total": total_images,
                    "eta_sec": eta_sec,
                    "desc": f"infer {done_images}/{total_images}",
                }
            )
...
def _organize_images(
    ...
    *,
    ...
    t0 = time.time()
    for idx, row in enumerate(tqdm(records, desc="organize", unit="img"), start=1):
        _control_wait_if_paused(control)
        if _control_should_stop(control):
            break
        if row.get("error"):
            ...
            progress_cb(
                {
                    ...
                    "total": total_rows,
                    "eta_sec": eta_sec,
                    "desc": f"organize {idx}/{total_rows}",
                }
            )
    return {"organized": ok, "failed": failed}


def run_from_config(config_path: Path, overrides: dict[str, object] | None = None, progress_cb=None, control=None) -> dict[str, Any]:
    logging.basicConfig(
        level=logging.INFO,
    ...
    records, stopped = _infer_records(
        images,
        input_dir=input_dir,
        runtime=runtime,
